...
description: HiveKeeper runs fine without an identity provider — pick solo, tenant-key, or OIDC.
---

The `hive-gateway` control plane supports user login via OpenID Connect, but **OIDC is entirely optional** —
it is gated behind the Spring `oidc` profile. Leave that profile off and there is no IdP dependency anywhere:
`OidcSecurityConfig`, the first-run `SetupService`, and the `IdpAdminClient` implementations are all
`@Profile("oidc")`, so without it `DefaultSecurityConfig` takes over (`permitAll`, with the controllers
enforcing the `X-Tenant-Key` service principal).

Two providers are supported, chosen with the `hivekeeper.idp` property: **Keycloak** (the default) and
**Authentik**. The gateway itself is provider-agnostic — it validates JWTs from whichever issuer you point it
at and calls the IdP's admin API only to create accounts. See [Authentik as the identity
provider](/authentik/) for what differs there.
...
| **OIDC** (per-user roles) | `SPRING_PROFILES_ACTIVE=postgres,oidc` + Keycloak or Authentik | OIDC login | JWT identity + DB-backed org/site/group roles |

## Solo mode — the simplest way to run without an identity provider
...
`SPRING_PROFILES_ACTIVE=postgres,oidc` (for Authentik instead, see [its page](/authentik/)). In that setup you
can also set `HIVEKEEPER_TENANTKEY_ENABLED=false` so a leaked static key cannot bypass per-user
authorization.
